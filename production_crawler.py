"""
PHARMYRUS V16 - PRODUCTION CRAWLER
Integrado com pool de 14 API keys para máxima performance
"""
import asyncio
import re
from typing import List, Dict, Set
from playwright.async_api import async_playwright, Page
from key_pool_manager import KeyPoolManager


class ProductionCrawler:
    """
    Crawler de produção com:
    - Pool de 14 API keys (5 WebShare + 3 ProxyScrape + 6 ScrapingBee)
    - Rotação automática de proxies
    - Extração WO + BR numbers
    - Stealth mode
    """
    
    def __init__(self):
        self.key_pool = KeyPoolManager()
        self.wo_pattern = re.compile(r'WO[\s/\-]?(\d{4})[\s/\-]?(\d{6})', re.IGNORECASE)
        self.br_pattern = re.compile(r'BR[\s/\-]?(\d+)', re.IGNORECASE)
        self.proxies = []
        self.current_proxy_index = 0
        
    async def initialize(self):
        """Initialize proxy pool"""
        print("\n🔧 Initializing proxy pool...")
        
        # Get proxies from all sources
        webshare = await self.key_pool.get_webshare_proxies()
        proxyscrape = await self.key_pool.get_proxyscrape_proxies()
        
        self.proxies = webshare + proxyscrape
        
        print(f"✅ Proxy pool ready: {len(self.proxies)} proxies")
        
        self.key_pool.print_status()
    
    def _get_next_proxy(self) -> str:
        """Get next proxy with round-robin"""
        if not self.proxies:
            return None
        
        proxy = self.proxies[self.current_proxy_index]
        self.current_proxy_index = (self.current_proxy_index + 1) % len(self.proxies)
        
        return proxy
    
    async def _setup_stealth(self, page: Page):
        """Setup stealth mode"""
        await page.add_init_script("""
            Object.defineProperty(navigator, 'webdriver', {get: () => undefined});
            window.chrome = {runtime: {}};
            Object.defineProperty(navigator, 'permissions', {
                get: () => ({
                    query: () => Promise.resolve({state: 'granted'})
                })
            });
        """)
    
    async def search_google_patents(self, query: str, use_proxy: bool = True) -> Set[str]:
        """Search Google Patents for WO numbers"""
        wo_numbers = set()
        
        max_attempts = 3
        for attempt in range(max_attempts):
            try:
                async with async_playwright() as p:
                    # Browser args
                    launch_args = {
                        'headless': True,
                        'args': [
                            '--disable-blink-features=AutomationControlled',
                            '--disable-dev-shm-usage',
                            '--no-sandbox'
                        ]
                    }
                    
                    # Add proxy if available
                    if use_proxy and self.proxies:
                        proxy = self._get_next_proxy()
                        if proxy:
                            launch_args['proxy'] = {'server': proxy}
                            print(f"   Using proxy: {proxy[:30]}...")
                    
                    browser = await p.chromium.launch(**launch_args)
                    page = await browser.new_page()
                    await self._setup_stealth(page)
                    
                    # Search
                    url = f"https://patents.google.com/?q={query}&num=20"
                    
                    try:
                        response = await page.goto(url, timeout=30000)
                        
                        if response.status == 200:
                            await page.wait_for_timeout(2000)
                            
                            content = await page.content()
                            
                            # Check for CAPTCHA
                            if 'captcha' in content.lower() or 'unusual traffic' in content.lower():
                                print(f"   ⚠️  CAPTCHA detected, rotating proxy...")
                                await browser.close()
                                continue
                            
                            # Extract WO numbers
                            matches = self.wo_pattern.findall(content)
                            for year, num in matches:
                                wo = f"WO{year}{num}"
                                wo_numbers.add(wo)
                            
                            print(f"   ✅ Found {len(wo_numbers)} WO numbers")
                            
                            await browser.close()
                            return wo_numbers
                        
                    except Exception as e:
                        print(f"   ⚠️  Attempt {attempt+1} failed: {str(e)[:50]}")
                        await browser.close()
                        continue
                    
            except Exception as e:
                print(f"   ❌ Error: {str(e)[:50]}")
                continue
        
        return wo_numbers
    
    async def get_wo_details(self, wo_number: str) -> Dict:
        """Get WO patent details and extract BR numbers"""
        br_numbers = set()
        
        try:
            async with async_playwright() as p:
                # Try with proxy
                launch_args = {
                    'headless': True,
                    'args': ['--disable-blink-features=AutomationControlled']
                }
                
                if self.proxies:
                    proxy = self._get_next_proxy()
                    if proxy:
                        launch_args['proxy'] = {'server': proxy}
                
                browser = await p.chromium.launch(**launch_args)
                page = await browser.new_page()
                await self._setup_stealth(page)
                
                # Search for WO number
                url = f"https://patents.google.com/?q={wo_number}"
                
                response = await page.goto(url, timeout=30000)
                
                if response.status == 200:
                    await page.wait_for_timeout(2000)
                    
                    content = await page.content()
                    
                    # Extract BR numbers from family
                    matches = self.br_pattern.findall(content)
                    for br_num in matches:
                        if len(br_num) >= 6:  # Valid BR number
                            br_numbers.add(f"BR{br_num}")
                    
                    print(f"   WO {wo_number}: {len(br_numbers)} BR patents")
                
                await browser.close()
                
        except Exception as e:
            print(f"   ⚠️  Error getting details for {wo_number}: {str(e)[:50]}")
        
        return {
            'wo_number': wo_number,
            'br_numbers': list(br_numbers)
        }
    
    async def search_molecule(self, molecule: str, dev_codes: List[str] = None) -> Dict:
        """
        Complete search for molecule:
        1. Search Google Patents for WO numbers
        2. For each WO, extract BR numbers
        """
        print(f"\n{'='*70}")
        print(f"🔬 SEARCHING: {molecule}")
        print(f"{'='*70}\n")
        
        all_wo_numbers = set()
        
        # Build queries
        queries = [
            f"{molecule} patent",
            f"{molecule} WO2011",
            f"{molecule} WO2016",
            f"{molecule} WO2018",
            f"{molecule} WO2020"
        ]
        
        if dev_codes:
            for code in dev_codes[:3]:
                queries.append(f"{code} patent")
        
        # Search for WO numbers
        print(f"📍 Searching {len(queries)} queries...")
        
        for i, query in enumerate(queries, 1):
            print(f"\n{i}/{len(queries)}: {query}")
            
            wo_numbers = await self.search_google_patents(query)
            all_wo_numbers.update(wo_numbers)
            
            await asyncio.sleep(1)  # Rate limit
        
        print(f"\n✅ Total WO numbers found: {len(all_wo_numbers)}")
        
        # Get BR numbers for each WO
        print(f"\n📍 Extracting BR numbers from {len(all_wo_numbers)} WOs...")
        
        all_br_numbers = set()
        wo_details = []
        
        for wo in sorted(all_wo_numbers):
            details = await self.get_wo_details(wo)
            wo_details.append(details)
            
            if details['br_numbers']:
                all_br_numbers.update(details['br_numbers'])
            
            await asyncio.sleep(1)  # Rate limit
        
        print(f"\n✅ Total BR numbers found: {len(all_br_numbers)}")
        
        # Print status
        self.key_pool.print_status()
        
        return {
            'molecule': molecule,
            'wo_numbers': sorted(list(all_wo_numbers)),
            'br_numbers': sorted(list(all_br_numbers)),
            'wo_details': wo_details,
            'summary': {
                'total_wo': len(all_wo_numbers),
                'total_br': len(all_br_numbers),
                'queries_executed': len(queries)
            }
        }


# TEST
async def test_crawler():
    """Test production crawler"""
    print("\n" + "="*70)
    print("TESTING PRODUCTION CRAWLER")
    print("="*70)
    
    crawler = ProductionCrawler()
    
    # Initialize
    await crawler.initialize()
    
    # Test with aspirin (simple molecule)
    result = await crawler.search_molecule(
        molecule='aspirin',
        dev_codes=[]
    )
    
    print(f"\n{'='*70}")
    print("RESULTS:")
    print(f"{'='*70}")
    print(f"WO numbers: {result['summary']['total_wo']}")
    print(f"BR numbers: {result['summary']['total_br']}")
    
    if result['wo_numbers']:
        print(f"\nWO NUMBERS:")
        for wo in result['wo_numbers'][:5]:
            print(f"  ✅ {wo}")
    
    if result['br_numbers']:
        print(f"\nBR NUMBERS:")
        for br in result['br_numbers'][:5]:
            print(f"  ✅ {br}")
    
    return len(result['wo_numbers']) > 0


if __name__ == "__main__":
    success = asyncio.run(test_crawler())
    exit(0 if success else 1)
